...
function Highlighter.highlight(props: HighlightProps)
	...
	if lineFolder == nil then
		...
		newLineFolder.Name = "SKXCMNTYHighlights"
...
local function C_49()
	...
	buttons.Execute.MouseButton1Up:Connect(function()
		if not injected then
			title.Text = "SKID X COMMUNITY - "..synversion.." (not injected! press attach)"
	...
	buttons.Attach.MouseButton1Up:Connect(function()
		if not injected then
			title.Text = "SKID X COMMUNITY - "..synversion.." (checking...)"
			task.wait(0.8)
			title.Text = "SKID X COMMUNITY - "..synversion.." (injecting...)"
			task.wait(2.3)
			title.Text = "SKID X COMMUNITY - "..synversion.." (checking whitelist...)"
			task.wait(1.6)
			title.Text = "SKID X COMMUNITY - "..synversion.." (scanning...)"
			task.wait(1.3)
			title.Text = "SKID X COMMUNITY- "..synversion.." (ready!)"
			injected = true
			task.wait(1)
			title.Text = "SKID X COMMUNITY - "..synversion
		else
			title.Text = "SKID X COMMUNITY - "..synversion.." (already injected!)"
			task.wait(1)
			title.Text = "SKID X COMMUNITY - "..synversion
		end
	end)
...
local function C_51()
	...
	script.Parent.savescript.MouseButton1Up:Connect(function()
		...
		makefolder("SKIDXCOMMUNITYSavedScripts")
		writefile("SKIDXCOMMUNITYScripts/"..script.Parent.scriptname.Text..".lua", module:GetActiveTextbox().Frame.Textbox.Text)
	end)
...
local function C_71()
	...
	yes.Background.ScrollingFrame.Dex.MouseButton1Up:Connect(function()
		...
		patches specifically for SKID X COMMUNITY.]]
	end)
